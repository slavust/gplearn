...
def generate_mul_tree(root_node_units, quantities_units, max_power, function_map, random_state):
    assert ['mul', 'pow'] in function_map.keys()
...
def generate_tree(root_node_units, quantities_units, depth, n_features, max_power, dimensional_functions_map, nondimensional_functions_list, random_state):
    dimensional_operator_names = ('mul', 'pow', 'add', 'sub')
    assert dimensional_operator_names in dimensional_functions_map.keys()
    ...
    if dimensional:
        ...
        if option_choice == 'add':
            ...
            return generate_mul_tree(root_node_units, quantities_units, max_power, random_state)
        assert False
    else:
        if required_to_build_minimal_tree:
        ...
        if option_choice == 'constant':
            ...
            operands = [generate_tree(
                        ...
                        max_power,
                        nondimensional_functions_list,
                        random_state) for _ in range(arity)]
...
def debug_render_tree(helper_tree):
    ...
    root_id = 'root'
    dot.node(root_id, helper_tree.node)
...
def build_program(
    ...
    if dimensional_function_names not in function_names:
